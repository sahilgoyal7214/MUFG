...
export default function AdvisorContent({ activeTab, isDark }) {
  ...
  // Enhanced chart builder state (similar to member dashboard)
  ...
  // Grid charts state for advisor dashboard
  const [gridCharts, setGridCharts] = useState([
    { id: 1, isConfigured: false }
  ]);
  ...
  // Data state for chart generation
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);

  // Load advisor-specific data and generate enhanced mock data
  useEffect(() => {
    const loadAdvisorData = async () => {
      try {
        ...
        // Generate enhanced mock data for charts (similar to member dashboard)
        setTimeout(() => {
  ...
  // Generate enhanced mock pension data (same as member dashboard)
