...
async function proxyRequest(request, method = 'GET', params) {
  try {
    ...
    const backendUrl = `${BACKEND_URL}/api/advisor${pathSegments}${queryParams ? `?${queryParams}` : ''}`;

    console.log('🔍 Advisor Dynamic Proxy:', {
    ...
    console.error('Advisor dynamic proxy error:', error);
    return NextResponse.json(
